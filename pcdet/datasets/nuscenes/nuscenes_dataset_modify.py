...
class NuScenesDataset(DatasetTemplate):
    ...
    def get_sweep(self, sweep_info):
        ...
        points_sweep = np.fromfile(str(lidar_path), dtype=np.float32, count=-1).reshape([-1, 6])[:,[0,1,2,3,5]]
    ...
    def __getitem__(self, index):
        ...
        if 'gt_boxes' in info:
            if self.dataset_cfg.get('FILTER_MIN_POINTS_IN_GT', False):
                ...
                mask = None
            # print('name = {}'.format(info['gt_names']))
            # info['gt_boxes'] = info['gt_boxes'][:,[0,1,2,3,4,5,6]]
            # print('gt_boxes = {}'.format(info['gt_boxes']))
            input_dict.update({
