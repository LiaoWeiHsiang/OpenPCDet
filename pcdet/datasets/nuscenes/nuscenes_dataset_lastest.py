...
class NuScenesDataset(DatasetTemplate):
    ...
    def __getitem__(self, index):
        ...
        import numpy as np 
        # voxels, coors, num_points_per_voxel = points_to_voxel_pillars(points,[0.1, 0.1, 0.2],[-51.2, -51.2, -5.0, 51.2, 51.2, 3.0],10,False,60000)
        
        # h_size = 1024
        # w_size = 1024
        # import numpy as np
        # # c_size = self.c_size
        # # img_coors = voxel_coords.cpu().numpy()[:,[0,1,2,3]] # -->[batch_size,c_size,h_size,w_size]
        # # print('img_coor = {}'.format(img_coor))
        # #img_3d = np.zeros([batch_size,c_size,h_size,w_size])
        # img_2d = np.zeros([h_size,w_size,3])
        # for img_coor in coors:
        #     # print(i)
        #     #img_3d[int(img_coor[0])][int(img_coor[1])][int(img_coor[2])][int(img_coor[3])] = 1   # img_3d = [1,1024,1024,40]
        #     img_2d[int(img_coor[1])][int(img_coor[0]),:] = [255,255,255 ]
        # # img_3d = torch.from_numpy(img_3d).cuda().float()
        # from PIL import Image
        # im = Image.fromarray(np.uint8(img_2d))
        # im.save('/data/OpenPCDet/122.png')
        # print('voxels = {}'.format(voxels))
        # print('coors = {}'.format(coors))
        # print('num_points_per_voxel = {}'.format(num_points_per_voxel))

        # import rospy
        # rospy.init_node('node', anonymous=True) 
        # def publish_point_cloud(pcl_pub,point_cloud,frame_id):
        #     header = Header()
        ...
        #         ]
        #     pcl_pub.publish(pcl2.create_cloud(header,fields,point_cloud[:,[0,1,2,4]]))
        # pcl_pub = rospy.Publisher('lidar',PointCloud2,queue_size=10)
        # #for i in range(10):
        # publish_point_cloud(pcl_pub,points,'sensor')
        ...
        is_key_frame = sample_data_record['is_key_frame']
            ...
            for record in records:
                ...
                for polygon in polygons:
                    ...
                    if "noise_rotation" in data_dict:
                        import math
                        noise_rotation = data_dict['noise_rotation']
                    # theta = np.radians(noise_rotation)
                        flip_xy = data_dict['flip_xy']
                        
                        if flip_xy[0]== True  and flip_xy[1]==True:
                            noise_rotation = noise_rotation #  
                        if flip_xy[0]==True and flip_xy[1]==False:
                            noise_rotation = 2*math.pi- noise_rotation
                        if flip_xy[0]==False and flip_xy[1]==True:
                            noise_rotation = 2*math.pi- noise_rotation
                        if flip_xy[0]==False and flip_xy[1]==False:
                            noise_rotation = noise_rotation
                        rotation_m = np.array(( (math.cos(noise_rotation), -math.sin(noise_rotation)),
                               (math.sin(noise_rotation),  math.cos(noise_rotation)) ))

                        # map_numpy  = np.array([x_axis_map_pt,y_axis_map_pt])

                        # map_numpy = map_numpy-512

                        exterior_array[:,:2] = exterior_array[:,:2].dot(rotation_m.T)

                        noise_scale = data_dict['noise_scale']

                        exterior_array = exterior_array*noise_scale
                    ...
                    if list(polygon.interiors):
                        ...
                        for interiors in list(polygon.interiors):
                            ...
                            if "noise_rotation" in data_dict:
                                rotation_m = np.array(((math.cos(noise_rotation), -math.sin(noise_rotation)),
                                       (math.sin(noise_rotation),  math.cos(noise_rotation)) ))
                                interiors_array[:,:2] = interiors_array[:,:2].dot(rotation_m.T)
                                interiors_array = interiors_array*noise_scale
                            #=============================================================
        ...
        if enlarge:
            ...
            # plt.savefig(os.path.join('/data/OpenPCDet/','6565.png'))
        # print(data)
        ...
        # plt.clf()   # Clear figure
        # plt.imshow(data, interpolation='nearest')
        # pcd_voxel_image = np.zeros
        x_axis = (points[:,0] + 51.2)
        y_axis = (points[:,1] + 51.2)

        # np.save(os.path.join('/data/OpenPCDet/pcd'), points)
        # plt.imshow(img_2d)
        ...
        #         if (row == [166, 206, 227]).all():
        #             x_axis_map_pt.append(j)
        #             # y_axis_map_pt.append(-i+1024)
        #             y_axis_map_pt.append(i)


        # plt.plot(x_axis_map_pt,y_axis_map_pt,'o',color = 'blue')
        # # print(data.shape)
        # x_axis_voxel_pt = []
        # y_axis_voxel_pt = []
        # for i,colum in enumerate(img_2d):
        #     for j,row in enumerate(colum):
        #         if row[0] == 255 and row[1]==255 and row[2] ==255:
        #             x_axis_voxel_pt.append(j)
        #             y_axis_voxel_pt.append(i)



        # plt.plot(x_axis_voxel_pt,y_axis_voxel_pt,'ro')
        # # plt.plot(x_axis_map_pt,y_axis_map_pt,'o',color = 'blue')
        # # plt.show()
        # plt.savefig(os.path.join('/data/OpenPCDet','121.png'))
        ...
        # # map_data = map_data
        # # map_data[0] = np.rot90(map_data[0])
        # #map_data[0] = np.rot90(map_data[0])
        # #map_data[0] = np.rot90(map_data[0])
        # # map_data[] = map_data

        # # map_data[0][]
        # for i,voxel_coord in enumerate(coors):

        #     # map_pixel = map_data[int(voxel_coord[0]),int(voxel_coord[2]),int(voxel_coord[3])]
        #     map_pixel = data[int(voxel_coord[1]),int(voxel_coord[0])]
        ...
        #         # print(i)
        #         #img_3d[int(img_coor[0])][int(img_coor[1])][int(img_coor[2])][int(img_coor[3])] = 1   # img_3d = [1,1024,1024,40]
        #     img_2d[int(voxel_coord[1])][int(voxel_coord[0]),:] = [255,255,255 ]
        #     # img_3d = torch.from_numpy(img_3d).cuda().float()
            
        #     if int(map_pixel[0]) ==166 and int(map_pixel[1]) ==206 and int(map_pixel[2]) ==227 :
        #         img_2d[int(voxel_coord[1])][int(voxel_coord[0]),:] = [166,206,227 ]
        #         # print('dskjdslkfj')
        #         # new_voxel_feature = torch.cat((voxel_feature, torch.tensor([1.]).cuda()), 0)
        #     # else:
        #     #     new_voxel_feature = torch.cat((voxel_feature, torch.tensor([0.]).cuda()), 0)
        # from PIL import Image
        # im = Image.fromarray(np.uint8(img_2d))
        # im.save('/data/OpenPCDet/124.png')
        ...
        #img.save(os.path.join('/data/OpenPCDet','127.png'))
