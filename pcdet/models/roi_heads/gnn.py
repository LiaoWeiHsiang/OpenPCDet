...
def multi_layer_fc_fn(Ks=[256, 64, 32, 64], num_classes=4, is_logits=False, num_layers=4):
...
class PointSetPooling_out(nn.Module):
    def __init__(self, point_MLP_depth_list=[256, 128, 64, 128, 256], output_MLP_depth_list=[256, 256, 256]):
        super(PointSetPooling_out, self).__init__()
    ...
    def forward(self, 
        ...
        extracted_features = self.point_linears(point_features) # N x 64
        ...
        # print(set_indices[:, 1])
        set_features = max_aggregation_fn(extracted_features, set_indices, keypoint_indices_len)

        # Step 3: MLP for set_features
        set_features = self.out_linears(set_features)
        return set_features
class PointSetPooling(nn.Module):
    def __init__(self, point_MLP_depth_list=[4, 32, 64, 128, 256], output_MLP_depth_list=[256, 256, 256]):
...
class GraphNetAutoCenter(nn.Module):
    def __init__(self, auto_offset=True, auto_offset_MLP_depth_list=[256, 64, 3], edge_MLP_depth_list=[259, 256, 256], update_MLP_depth_list=[256, 256, 256]):
